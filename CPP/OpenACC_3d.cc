...
const float dz = Lz/(nz-1);

// Chosen as 1% of the courant number limit for dt
const float dt =  0.01*(dx+dy);//0.001;
//const float courant = 3.0*dt/dx;
...
#pragma data pcopyin(u[0:nx][0:ny][0:nz]) copyout(ucpy[0:nx][0:ny])
    {
#pragma acc kernels
    {
...
{
#pragma data pcopyin(u[0:nx][0:ny][0:nz], v[0:nx][0:ny][0:nz]) copyout(b[0:nx][0:ny][0:nz])
    {
#pragma acc kernels
    {
    #pragma acc loop independent vector
...
#pragma data pcopyin(pn[0:nx][0:ny][0:nz]) copyout(p[0:nx][0:ny][0:nz])
    {
#pragma acc kernels
        {
        #pragma acc loop independent vector
...
{
#pragma data pcopyin(p[0:nx][0:ny][0:nz], vn[0:nx][0:ny][0:nz]) copyout(v[0:nx][0:ny][0:nz])
    {
#pragma acc kernels
    {
    #pragma acc loop independent vector
...
{
#pragma data pcopyin(p[0:nx][0:ny][0:nz], wn[0:nx][0:ny][0:nz) copyout(w[0:nx][0:ny][0:nz])
    {
#pragma acc kernels
    {
    #pragma acc loop independent vector
...
{
    float * __restrict__ un, * __restrict__ vn, * __restrict__ wn;
    for(int n = 0; n<nt;n++)
    {
...
int main()
{
    float * __restrict__ u = (float*)calloc(nx*ny*nz,sizeof(float));
    float * __restrict__ v = (float*)calloc(nx*ny*nz,sizeof(float));
    float * __restrict__ w = (float*)calloc(nx*ny*nz,sizeof(float));
    float * __restrict__ p = (float*)calloc(nx*ny*nz,sizeof(float));
    float * __restrict__ b = (float*)calloc(nx*ny*nz,sizeof(float));
