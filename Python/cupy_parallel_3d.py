...
if __name__ == "__main__":
    ...
    # y   = np.arange(0,Ly,dy)
    # z   = np.arange(0,Lz,dz)
    u   = cp.zeros((new_nz,ny,nx))#, dtype = np.float32 )
    v   = cp.zeros((new_nz,ny,nx))#, dtype = np.float32 )
    w   = cp.zeros((new_nz,ny,nx))#, dtype = np.float32 )
    p   = cp.zeros((new_nz,ny,nx))#, dtype = np.float32 )
    b   = cp.zeros((new_nz,ny,nx))#, dtype = np.float32 )
    un  = cp.zeros((new_nz,ny,nx))#, dtype = np.float32 )
    vn  = cp.zeros((new_nz,ny,nx))#, dtype = np.float32 )
    wn  = cp.zeros((new_nz,ny,nx))#, dtype = np.float32 )
    pn  = cp.zeros((new_nz,ny,nx))#, dtype = np.float32 )
