...
if __name__ == '__main__':
    ...
    u   = cp.zeros((ny,nx), dtype = np.float64)
    v   = cp.zeros((ny,nx), dtype = np.float64)
    p   = cp.zeros((ny,nx), dtype = np.float64)
    b   = cp.zeros((ny,nx), dtype = np.float64)

    start_time = time.time()
    u,v,p = cavity_flow(nt,u,v,p,b,vis,rho,nx,ny,nit, dt, dx, dy)

    u_cpu, v_cpu, p_cpu = cp.asnumpy(u), cp.asnumpy(v), cp.asnumpy(p)
    ...
    with PdfPages('plot_serial.pdf') as pdf:
        plt.contourf(x, y, p_cpu, alpha=0.5, cmap=cm.viridis)
        plt.colorbar()
        plt.contour(x, y, p_cpu, cmap=cm.viridis)
        plt.streamplot(x, y, u_cpu, v_cpu)
        plt.xlabel('X')
        plt.ylabel('Y')
        pdf.savefig(dpi = 300)
        plt.close()
