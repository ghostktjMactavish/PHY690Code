...
def cavity_flow(nt,u,v,w,p,b,vis,nit):
    ...
    for n in tqdm(range(nt)):
        ...
        us = cp.sign(un[1:-1,1:-1,1:-1])
        vs = cp.sign(vn[1:-1,1:-1,1:-1])
        ws = cp.sign(wn[1:-1,1:-1,1:-1])
...
if __name__=="__main__":
    ...
    i_dy = 1/dy
    i_dz = 1/dz
    u   = cp.zeros((nz,ny,nx))#, dtype = np.float32 )
    v   = cp.zeros((nz,ny,nx))#, dtype = np.float32 )
    w   = cp.zeros((nz,ny,nx))#, dtype = np.float32 )
    p   = cp.zeros((nz,ny,nx))#, dtype = np.float32 )
    b   = cp.zeros((nz,ny,nx))#, dtype = np.float32 )
    un   = cp.zeros((nz,ny,nx))#, dtype = np.float32 )
    vn   = cp.zeros((nz,ny,nx))#, dtype = np.float32 )
    wn   = cp.zeros((nz,ny,nx))#, dtype = np.float32 )
    pn   = cp.zeros((nz,ny,nx))#, dtype = np.float32 )
    ...
    u_cpu, v_cpu, w_cpu, p_cpu = cp.asnumpy(u), cp.asnumpy(v), cp.asnumpy(w), cp.asnumpy(p)
